...
The mobile application flow is `ServerConnection -> Home -> NewCombatParticipants -> NewCombatScenario -> NewCombatRules -> NewCombatReview -> Combat`, with `AddParticipants`, `CreationHub`, and `Settings` as secondary stack destinations. Home has Continue Combat only for a recoverable session, New Combat, Create, and Settings; it intentionally has no Exit action. Setup owns a versioned local draft of IDs, encounter instance keys, sides, controllers, scenario, and supported rule configuration. Review calls server validation before the real combat-session creation request. Creation Hub visibly disables Character, Companion, and Creature creation; no editor or creation API exists yet. Settings persists presentation/audio preferences, including master volume, and manages the configured server URL and local recovery reference.
