...
function StepButtons({ navigation, next, nextLabel = "Next", disabled = false }) {
  return <View><PrimaryButton title="Back" variant="ghost" onPress={() => navigation.goBack()} /><PrimaryButton title={nextLabel} onPress={() => navigation.navigate(next)} disabled={disabled} /></View>;
}
...
export function ParticipantsScreen({ navigation }) {
  ...
  return <Shell step={1} title="Compose the encounter" subtitle="Choose real server content, then assign each participant independently to a side and controller.">
    ...
    <PrimaryButton title="Add from catalog" variant="secondary" onPress={() => navigation.navigate("AddParticipants")} />
    <PrimaryButton title="Create new · coming later" variant="ghost" disabled onPress={() => {}} />
...
export function AddParticipantsScreen({ navigation }) {
  ...
  function Addable({ entry }) { return <View><Choice entry={entry} onPress={() => add(entry, PARTY)} /><PrimaryButton title="Add to opponents" variant="secondary" compact onPress={() => add(entry, HOSTILES)} /></View>; }
  if (loading) return <SafeAreaView style={styles.safe}><ActivityIndicator color={colors.cyan} style={{ marginTop: 50 }} /></SafeAreaView>;
  return <Shell step={1} title="Add participant" subtitle="Choose a real character or creature, then return to composition. Duplicate templates receive separate encounter instance keys."><Text style={styles.sectionTitle}>Characters</Text>{catalog.characters.map((entry) => <Addable key={`character-${entry.id}`} entry={entry} />)}<Text style={styles.sectionTitle}>Creatures</Text>{catalog.creatures.map((entry) => <Addable key={`creature-${entry.id}`} entry={entry} />)}<PrimaryButton title="Open Creation Hub" variant="secondary" onPress={() => navigation.navigate("CreationHub", { returnTo: "AddParticipants" })} /><StatusMessage error message={error} /><PrimaryButton title="Back to participants" variant="ghost" onPress={() => navigation.goBack()} /></Shell>;
}
